...
def set_combo_box():
    ...
    scrollbar.configure(columns=("#1","#2","#3","#4"))
    ...
    scrollbar.heading("#4",text = "Advance", anchor=W)
...
def delete():       
    ...
    query = "delete from advance where name = %s and date = %s"
...
def search():
    ...
    if sv2.get() == "" and sv3.get() == "":
        if n.get() == "All":
            ...
            query = "select * from advance where id = %s"
    ...
    elif sv2.get() != "" and sv3.get() != "":
        if n.get() == "All":
            query = "select * from advance where MONTH(date) = %s AND YEAR(date) = %s"
            ...
            query = "select * from advance where id = %s AND MONTH(date) = %s AND YEAR(date) = %s"
        ...
        if n.get() == "All":
            query = "select * from advance where YEAR(date) = %s"
            ...
            query = "select * from advance where id = %s AND YEAR(date) = %s"
...
view_att_lbl = Label(window, text = "ADVANCES", font = ("yu gothic ui",50,"bold"),bg = "white",fg = "#00337C")
view_att_lbl.place(x = 500, y = 5)
...
delete_button.place(x = 1220, y = 157)
